import { Zap, Mail, Phone, MapPin, ArrowRight, Linkedin, Twitter, Github, Globe, Award, Users, Clock, Shield } from 'lucide-react'
        ...
        <div className="theme-container" style={{paddingTop: 'var(--space-4xl)', paddingBottom: 'var(--space-2xl)'}}>
          ...
          <div className="grid lg:grid-cols-4 gap-12 mb-16">
            {footerSections.map((section, index) => (
              <div key={index}>
                <h3 className="text-lg font-bold text-white mb-6" style={{fontFamily: 'var(--font-sans)'}}>
                  {section.title}
                </h3>
                <ul className="space-y-4">
                  {section.links.map((link, linkIndex) => (
                    <li key={linkIndex}>
                      <a 
                        href={link.href} 
                        className="text-gray-300 hover:text-emerald-300 transition-colors text-sm leading-relaxed group flex items-start space-x-2"
                        style={{transitionDuration: 'var(--duration-normal)'}}
                      >
                        <ArrowRight className="w-3 h-3 mt-1 text-emerald-400 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all flex-shrink-0" style={{transitionDuration: 'var(--duration-normal)'}} />
                        <span>{link.name}</span>
                      </a>
                    </li>
                  ))}
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            <div className="flex items-center space-x-4 p-6 bg-slate-800/30 hover:bg-slate-700/30 transition-colors" style={{borderRadius: 'var(--radius-2xl)', transitionDuration: 'var(--duration-normal)'}}>
              <div className="w-12 h-12 bg-emerald-500/20 flex items-center justify-center" style={{borderRadius: 'var(--radius-xl)'}}>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
                <input 
                  ...
                  className="flex-1 px-6 py-4 bg-slate-800 border border-gray-600 text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all"
          ...
          <div className="theme-container py-8">
            <div className="flex flex-col lg:flex-row justify-between items-center space-y-6 lg:space-y-0">
                  ...
                  {socialLinks.map((social, index) => {
                    ...
                    return (
                      <a 
                        ...
                        className="w-10 h-10 bg-slate-800 hover:bg-emerald-500 flex items-center justify-center transition-all group"
              ...
              <div className="flex items-center space-x-6 text-sm">
                <a href="#privacy" className="text-gray-400 hover:text-emerald-300 transition-colors" style={{transitionDuration: 'var(--duration-normal)'}}>
                  Privacy Policy
                </a>
                <a href="#terms" className="text-gray-400 hover:text-emerald-300 transition-colors" style={{transitionDuration: 'var(--duration-normal)'}}>
                  Terms of Service
                </a>
                <a href="#security" className="text-gray-400 hover:text-emerald-300 transition-colors" style={{transitionDuration: 'var(--duration-normal)'}}>
                  Security
                </a>
              </div>
            </div>
